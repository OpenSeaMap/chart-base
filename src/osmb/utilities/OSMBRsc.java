...
public class OSMBRsc
{
	...
	/**
	 * loads the strings for the default locale
	 */
	public static void initializeRSTRs()
	{
		initializeRSTRs(getLocale());
	}
	...
	public static void initializeRSTRs(Locale newLocale)
	{
		try
		{
			STRING_RESOURCE = ResourceBundle.getBundle("osmb.resources.text.loc-nls", newLocale);
	...
	/**
	 * loads the images from imgs.
	 * 
	 * @param newLocale
	 */
	public static void initializeRSCs(Locale newLocale)
	{
		try
		{
			IMAGE_RESOURCE = ResourceBundle.getBundle("osmb.resources.images.imgs", newLocale);
			currLocale = newLocale;
		}
		catch (MissingResourceException e)
		{
	...
	public static InputStream getResourceAsStream(String name, String extension)
	{
		getLocale();
		InputStream in;
		in = OSMBRsc.class.getResourceAsStream(String.format("%s_%s_%s.%s", name, localeLanguage, localeCountry, extension));
		if (in != null)
			return in;
		in = OSMBRsc.class.getResourceAsStream(String.format("%s_%s.%s", name, localeLanguage, extension));
		if (in != null)
			return in;
		in = OSMBRsc.class.getResourceAsStream(String.format("%s.%s", name, extension));
		return in;
	}
}
