...
public class EnvironmentSetup
{
	...
	public static void checkMemory()
	{
		...
		{
			String msg = String.format(OSMBStrs.RStr("msg_environment_lack_memory"), heapMBFormatted);
			JOptionPane.showMessageDialog(null, msg, OSMBStrs.RStr("msg_environment_lack_memory_title"), JOptionPane.WARNING_MESSAGE);
		}
	}
	...
	protected static void checkDirectory(File dir, String dirName, boolean checkIsWriteable)
	{
		try
		{
			OSMBUtilities.mkDirs(dir);
		}
		catch (IOException e)
		{
			// GUIExceptionHandler
			// .processFatalExceptionSimpleDialog(String.format(OSMBStrs.RStr("msg_environment_error_create_dir"), dirName, dir.getAbsolutePath()), e);
		}
		if (!checkIsWriteable)
			return;
		try
		{
			...
			File testFile = File.createTempFile("OSMB-Test", "", dir);
		...
		{
			// GUIExceptionHandler
			// .processFatalExceptionSimpleDialog(String.format(OSMBStrs.RStr("msg_environment_error_write_file"), dirName, dir.getAbsolutePath()), e);
		}
	}
}
